...
function createPromise(type, url, data, config, socket, cache = {}) {
	...
	return new Promise(function (resolve, reject) {
		if (cacheData) {
			...
			const http = ['get', 'head', 'delete'].includes(type) ? axios[type](url, { ...config, params: data }) : axios[type](url, data, config)
			http
				.then(function (res) {
...
export default function (options) {
	...
	let cache = config.cache ? { key: createKey(url, type, config.params, data), ...config.cache } : {}
