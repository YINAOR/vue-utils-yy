...
let lunarInfo = [
	...
	42160, 46261, 27216, 27968, 109396, 11104, 38256, 21234, 18800, 25958, 54432, 59984, 28309, 23248, 11104, 100067, 37600, 116951, 51536, 54432,
	120998, 46416, 22176, 107956, 9680, 37584, 53938, 43344, 46423, 27808, 46416, 86869, 19872, 42448, 83315, 21200, 43432, 59728, 27296, 44710, 43856,
	19296, 43748, 42352, 21088, 62051, 55632, 23383, 22176, 38608, 19925, 19152, 42192, 54484, 53840, 54616, 46400, 46496, 103846, 38320, 18864, 43380,
	42160, 45690, 27216, 27968, 44870, 43872, 38256, 19189, 18800, 25776, 29859, 59984, 27480, 21952, 43872, 38613, 37600, 51552, 55636, 54432, 55888,
	30034, 22176, 43959, 9680, 37584, 51893, 43344, 46240, 47780, 44368, 21977, 19360, 42416, 86390, 21168, 43312, 31060, 27296, 44368, 23378, 19296,
	42726, 42208, 53856, 60005, 54576, 23200, 30371, 38608, 19415, 19152, 42192, 118966, 53840, 54560, 56645, 46496, 22224, 21938, 18864, 42359, 42160,
	43600, 111189, 27936, 44448,
]
...
let sTermInfo = [
	0, 21208, 42467, 63836, 85337, 107014, 128867, 150921, 173149, 195551, 218072, 240693, 263343, 285989, 308563, 331033, 353350, 375494, 397447,
...
function sTerm(C, B) {
	let A = new Date(31556925974.7 * (C - 1900) + sTermInfo[B] * 60000 - 2208549300000)
	return A.getUTCDate()
}
...
export function Calendar(N, F, tY, tM, tD) {
	...
	for (let J = 0; J < this.length; J++) {
		...
		this[J] = new CalElement(
			N,
			F + 1,
			J + 1,
			nStr1[(J + this.firstWeek) % 7],
			L,
			B,
			K++,
			C,
	...
	for (let J in wFtv) {
		if (wFtv[J].match(/^(\d{2})(\d)(\d)([\s\\*])(.+)$/)) {
			if (Number(RegExp.$1) === F + 1) {
				H = Number(RegExp.$2)
				G = Number(RegExp.$3)
				this[(this.firstWeek > G ? 7 : 0) + 7 * (H - 1) + G - this.firstWeek] &&
					(this[(this.firstWeek > G ? 7 : 0) + 7 * (H - 1) + G - this.firstWeek].solarFestival += RegExp.$5 + ' ')
			}
		}
	}
	for (let J in lFtv) {
...
export function getFestivals(start, end) {
	...
	let endMonth = new Date(endDate.getFullYear(), endDate.getMonth())
	while (endMonth.getTime() > startMonth.getTime()) {
		startMonth = dateFormat(startMonth, null, { M: 1 })
		let festivals = getFestivalsByMonth(startMonth.getFullYear(), startMonth.getMonth())
