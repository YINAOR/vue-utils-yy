...
export default function (item = {}) {
	...
	// 只匹配path，忽略参数
	let regex = new RegExp(oRegex.source.replace('(?:\\/)?$', '($|\\?)').replace(/\[\^\d\\\/\]/, '[^\\/]'), 'i')
	// 设置拦截请求ur
	...
	Mock.mock(regex, item.method, function (options) {
		...
		let query = Object.assign({}, item.params || {}, getParams(item.url, options.url), parse(options.url || ' '), body, toObject(body.data) || {})
